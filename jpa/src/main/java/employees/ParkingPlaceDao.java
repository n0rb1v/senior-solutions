...
public class ParkingPlaceDao {
    ...
    public ParkingPlace findParkingPlace(int number) {
        EntityManager entityManager = entityManagerFactory.createEntityManager();
        ParkingPlace parkingPlace = entityManager.createQuery("select p from ParkingPlace p where p.number = :number",ParkingPlace.class)
